...
# doctypes used by the hide-from-attendance filter (sa-print-attendance-
# enrichments). Declaring the dependency here does not install the app —
# that still needs `bench get-app hrms` + `bench --site <site> install-app
# hrms` run against a real bench before the doctypes actually exist.
