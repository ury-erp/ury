...
DEMO_PLAN_NAME = "dev-seed-more-seed-plan"


def _ensure_demo_plan(branch_name, company_name):
    if frappe.db.exists("URY Sales Plan", DEMO_PLAN_NAME):
...
def _ensure_demo_issue_authorization(branch_name, company_name, department, component_item, authorized_qty):
...
def _seed_wastage(branch_name, company_name, departments, items, actor):
    ...
    for i in range(WASTAGE_ROW_COUNT):
        ...
        issue_authorization = _ensure_demo_issue_authorization(
            branch_name, company_name, department, component_item, authorized_qty
        )
