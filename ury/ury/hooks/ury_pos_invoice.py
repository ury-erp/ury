...
def calculate_and_set_times(doc, method):
    ...
    # Dono strings ko datetime object mein convert karne ka sahi tarika
    current_time = datetime.strptime(current_time_str, "%Y-%m-%d %H:%M:%S.%f")
    creation_time = datetime.strptime(str(doc.creation), "%Y-%m-%d %H:%M:%S.%f")
    
    # Time difference nikalna
    time_difference = current_time - creation_time
    
    # Total seconds nikal kar time format karna
