...
class URYDailyPandL(Document):
        def cogs_sold(self):
                ...
                unset_item_prices = []
                # Batch-fetch all item prices for non-PB items (avoids N+1)
                non_pb_item_codes = [item['Item Code'] for item in non_pb_item_sales]
                item_prices_map = {}
                if non_pb_item_codes:
                    price_rows = frappe.db.get_all(
                        "Item Price",
                        fields=['item_code', 'price_list_rate'],
                        filters={'price_list': report_settings.buying_price_list, 'item_code': ('in', non_pb_item_codes)}
                    )
                    item_prices_map = {r.item_code: r.price_list_rate for r in price_rows}

                for item in non_pb_item_sales:
                        item_name = item['Item Name']
                        rate = item_prices_map.get(item['Item Code'])
                        if not rate:
