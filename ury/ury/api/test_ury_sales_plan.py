...
class TestSalesPlanSubmittableLifecycle(TestSalesPlanWorkflowTransitions):
    ...
    def test_full_walk_to_cancelled_executes_and_audits_every_hop(self):
        ...
        # Supersede/Cancel now requires "URY Sales Plan Controller" plus a
        # reason (see _guard_backward_transition) -- grant the role here too
        # rather than switching users mid-walk, since this test's own point
        # is the full docstatus/audit walk, not role separation (that has
        # its own dedicated coverage above).
        frappe.get_doc("User", TEST_SALES_PLAN_MANAGER).add_roles("URY Sales Plan Controller")

        frappe.set_user(TEST_SALES_PLAN_MANAGER)
        try:
            previous = "Draft"
            for hop, target in enumerate(self.FULL_WALK, start=1):
                reason = "full walk test cancel" if target == "Superseded/Cancelled" else None
                result = transition_plan(name=name, target_state=target, reason=reason)
    ...
    def test_save_draft_rejects_a_proposed_plan_even_though_docstatus_is_0(self):
        """Proposed is doc_status 0 (still freely "save"-able as far as
        Frappe itself is concerned), but save_draft()'s own gate is tighter
        than the raw docstatus check: per the accepted "items editable only
        in Draft" design, a Proposed plan is already a claim made to someone
        else in the approval chain, not a private scratchpad -- editing it
        must go through the explicit, audited "Return to Draft" workflow
        action, not silently through this endpoint."""
        from ury.ury.api.ury_sales_plan import save_draft, transition_plan
        ...
        message = str(ctx.exception)
        self.assertIn("can no longer be saved as a draft", message)
        self.assertIn("Return to Draft", message)
...
class TestBackfillSalesPlanDocstatusPatch(TestSalesPlanWorkflowTransitions):
    ...
    def test_backfilled_row_can_then_actually_be_cancelled(self):
        ...
        # Supersede/Cancel requires "URY Sales Plan Controller" plus a reason
        # (see _guard_backward_transition) -- irrelevant to what this test is
        # actually pinning (the docstatus backfill), so satisfy it plainly.
        frappe.set_user(TEST_SALES_PLAN_CONTROLLER)
        try:
            result = transition_plan(
                name=name, target_state="Superseded/Cancelled", reason="backfill regression test cancel"
            )
        finally:
            frappe.set_user("Administrator")
