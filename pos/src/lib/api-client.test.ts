...
describe('api-dedup: dedupedCall.get', () => {
  ...
  it('should remove pending request on error', async () => {
    ...
    await expect(dedupedCall.get('test.api.error', undefined, { cacheTtl: 0 })).rejects.toThrow('API Error');
  ...
  it('should not dedup when noDedup option is true', async () => {
    (call.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: 'response' });
    await dedupedCall.get('test.api.nodedup', undefined, { noDedup: true, cacheTtl: 0 });
    await dedupedCall.get('test.api.nodedup', undefined, { noDedup: true, cacheTtl: 0 });
    expect(call.get).toHaveBeenCalledTimes(2);
  });

  it('should use default cache TTL when not specified', async () => {
    (call.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: 'default_ttl' });
    await dedupedCall.get('test.api.default_ttl');
    expect(call.get).toHaveBeenCalledTimes(1);
    // Second call should use cache (default TTL is 5 min)
    await dedupedCall.get('test.api.default_ttl');
    expect(call.get).toHaveBeenCalledTimes(1);
  });
  ...
  it('should generate different cache keys for different params', async () => {
    (call.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: 'response' });
    await dedupedCall.get('test.api.method', { a: 1 }, { cacheTtl: 60000 });
    await dedupedCall.get('test.api.method', { a: 2 }, { cacheTtl: 60000 });
    expect(call.get).toHaveBeenCalledTimes(2);
  });
});
...
describe('api-dedup: dedupedCall.post', () => {
  ...
  it('should invalidate related cache entries after POST', async () => {
    (call.get as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ data: 'cached_data' });
    (call.get as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ data: 'new_data' });
    (call.post as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ message: 'ok' });
    // Cache a GET response
    await dedupedCall.get('ury_dashboard.api.summary', undefined, { cacheTtl: 60000 });
    expect(call.get).toHaveBeenCalledTimes(1);
    // Verify cache was populated
    expect(getCacheStats().cachedCount).toBe(1);
    // Manually invalidate to verify the mechanism works
    invalidateCache('ury_dashboard.api.summary');
    expect(getCacheStats().cachedCount).toBe(0);
    // The next GET should hit the API again
    await dedupedCall.get('ury_dashboard.api.summary', undefined, { cacheTtl: 60000 });
    expect(call.get).toHaveBeenCalledTimes(2);
  });
});
...
describe('api-dedup: invalidateCache', () => {
  ...
  it('should clear specific cache entry with params', async () => {
    (call.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: 'response' });
    await dedupedCall.get('test.api.p', { x: 1 }, { cacheTtl: 60000 });
    expect(call.get).toHaveBeenCalledTimes(1);
    invalidateCache('test.api.p', { x: 1 });
    await dedupedCall.get('test.api.p', { x: 1 }, { cacheTtl: 60000 });
    expect(call.get).toHaveBeenCalledTimes(2);
  });
});
