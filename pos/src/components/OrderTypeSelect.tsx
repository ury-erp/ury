...
const OrderTypeSelect = ({ disabled }: OrderTypeSelectProps) => {
  ...
  const getTranslatedLabel = (orderType: OrderType) => {
    const labelMap: Record<OrderType, string> = {
      "Dine In": t('order.dineIn'),
      "Take Away": t('order.takeaway'),
      "Delivery": t('order.delivery'),
      "Phone In": t('order.phoneIn'),
      "Aggregators": t('order.aggregators')
    };
        ...
        {ORDER_TYPES.map(({ label, value, icon: Icon }) => {
          ...
          return (
            <Button
              ...
              disabled={isDisabled}
              title={isDineIn && isRestrictedFromTableOrders ? t('order.dineInRestricted', 'Dine In is not available for your role') : undefined}
            >
