...
const OrderTypeSelect = ({ disabled }: OrderTypeSelectProps) => {
  const { selectedOrderType, setSelectedOrderType, selectedTable, posProfile, isUpdatingOrder, orderTypes } = usePOSStore();
  ...
  return (
    <div>
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-2 px-2">
        {ORDER_TYPES
          .filter(type => orderTypes.length === 0 || orderTypes.includes(type.value))
          .map(({ value, icon: Icon }) => {
