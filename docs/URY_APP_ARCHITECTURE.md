...
## 9. Local Development & Tracks Workflow

**Environment**:
The actual Frappe bench is in WSL (Ubuntu):
- Bench: `/mnt/c/Users/swafa/Projects/Bench/ury-bench`
- Site: `/mnt/c/Users/swafa/Projects/Bench/ury-bench/sites/ury.local`
- App: `/mnt/c/Users/swafa/Projects/Bench/ury-bench/apps/ury`

**Command Execution**:
Run all `bench` and `yarn` commands using WSL. For example: 
`wsl --cd /mnt/c/Users/swafa/Projects/Bench/ury-bench bench clear-cache`.

**Tracks Strategy**:
We do not use git branches in this management workspace for tracking tasks. Instead, all work is organized into sub-directories within the `tracks/` folder. The workspace root `index.md` tracks the status of all initiatives. When working on a task, document it in `tracks/<task-name>/` and implement changes directly in the WSL app path.
