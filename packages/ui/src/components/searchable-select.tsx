...
export function SearchableSelect({
  ...
  const selectedOption = useMemo(() => {
    ...
    return options.find(
      (opt) =>
        (opt.value !== undefined && String(opt.value).toLowerCase() === valLower) ||
        (opt.label !== undefined && String(opt.label).toLowerCase() === valLower)
    );
  }, [options, value]);
  ...
  useEffect(() => {
    if (!isTyping) {
      setSearchTerm(selectedOption ? selectedOption.label : (value !== undefined && value !== null ? String(value) : ''));
    }
  ...
  const filteredOptions = useMemo(() => {
    ...
    return options.filter(
      (opt) =>
        ...
        (opt.value && String(opt.value).toLowerCase().includes(term))
    );
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const matched = options.find(
      (opt) => opt.label.toLowerCase() === val.toLowerCase() || String(opt.value).toLowerCase() === val.toLowerCase()
    );
    if (matched) {
      ...
      {filteredOptions.length > 0 ? (
        filteredOptions.map((opt) => {
          ...
          const isSelected =
            ...
            (opt.value === value ||
              opt.label === value ||
              (opt.value !== '' && opt.value && value !== '' && value && String(opt.value).toLowerCase() === String(value).toLowerCase()) ||
              (opt.label !== '' && opt.label && value !== '' && value && String(opt.label).toLowerCase() === String(value).toLowerCase()));
