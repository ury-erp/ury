...
/**
 * POS open flag from `get_captain_context()`.
 * Multi-cashier profiles use room-scoped `pos_opening_check`; single-cashier
 * uses branch-wide `posOpening`. `null` opening_state on the parent context
 * means the server could not resolve open status (fail closed).
 */
