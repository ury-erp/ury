...
export async function networkPrint(orderId: string, printer: string, printFormat: string) {
  const response = await call.post('ury.ury.api.ury_print.network_printing', {
    doctype: 'POS Invoice',
    name: orderId,
    printer_setting: printer,
    print_format: printFormat,
  });
  return (response as { message?: unknown })?.message ?? response;
}

export async function selectNetworkPrinter(orderId: string, posProfile: string, printFormat?: string | null) {
  const response = await call.post('ury.ury.api.ury_print.select_network_printer', {
    invoice_id: orderId,
...
/** Socket / branch print station transport. Marks invoice printed server-side. */
...
export function assertNetworkPrintSuccess(result: unknown): void {
  const text =
...
export async function getLinkedMergeSecondaries(): Promise<string[]> {
  ...
  return rows
    .map((row: { custom_merged_pos_invoice?: string | null }) => row.custom_merged_pos_invoice)
