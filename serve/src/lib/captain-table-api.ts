...
export interface BranchCaptain {
  name: string;
  label: string;
}

/** Captains assigned on Branch.user (URY User child table), narrowed vs all Users. */
export async function getBranchCaptains(
  branch: string,
  opts?: { search?: string; exclude?: string; limit?: number }
): Promise<BranchCaptain[]> {
  const limit = opts?.limit ?? 20;
  const search = (opts?.search || '').trim().toLowerCase();
  const branchDoc = (await db.getDoc('Branch', branch)) as {
  ...
  const userIds = Array.from(
    new Set(
      (branchDoc.user || [])
        .map((row) => row.user)
        .filter((name): name is string => Boolean(name) && name !== opts?.exclude)
    )
  );
  ...
  const rows = (await db.getDocList('User', {
    ...
    limit: Math.max(userIds.length, limit),
    asDict: true,
  } as unknown as Parameters<typeof db.getDocList>[1])) as Array<{
    name: string;
    full_name?: string;
  ...
  return rows
    .map((row) => ({ name: row.name, label: row.full_name || row.name }))
    .filter((row) => {
