...
/** True only when sync returned a POS Invoice with a document name (safe to clear local draft). */
export function getSyncedInvoiceName(message: SyncOrderResponse | null | undefined): string | null {
  if (!message || typeof message !== 'object') return null;
  if ('status' in message && (message as { status?: string }).status === 'Failure') return null;
  const name = (message as { name?: unknown }).name;
  return typeof name === 'string' && name.trim() ? name : null;
}

export function isSyncFailure(message: SyncOrderResponse | null | undefined): boolean {
  return Boolean(
    message &&
      typeof message === 'object' &&
      'status' in message &&
      (message as { status?: string }).status === 'Failure'
  );
}
...
export async function getSplitGroup(invoice: string): Promise<unknown[]> {
  const res = await call.get("ury.ury_pos.api.get_split_group", { invoice });
