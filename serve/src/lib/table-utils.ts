...
export function isTableOccupied(table: Table | null | undefined): boolean {
  return Number(table?.occupied) === 1;
}

/** RN `canUnmergeTable`: every member of the merge cluster must be free. */
export function canUnmergeTable(table: Table, allTables: Table[]): boolean {
  if (!isMergedTable(table)) return false;
  const members = getMergeGroupMembers(table, allTables);
  const byName = new Map(allTables.map((row) => [row.name, row]));
  return members.every((name) => {
    const member = byName.get(name);
    return member ? !isTableOccupied(member) : false;
  });
}

/**
 * Table-level merge/unmerge affordances (RN `getTableActions` subset).
 * Serve does not offer bill-merge — only `merge_tables_batch` / `unmerge_tables`.
 */
export function getTableMergeActions(table: Table, allTables: Table[]) {
  return {
    showMerge: true,
    showUnmerge: canUnmergeTable(table, allTables),
  };
}
