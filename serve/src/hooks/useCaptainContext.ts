 ...
 * (`get_captain_context()`) and derives client-side capability flags via
 * `derivePOSCapabilities`. Opening state is authoritative from the backend
 * (room-scoped for multi-cashier). `canOpenPOS` / `canClosePOS` stay false —
 * Serve never hosts opening/closing forms.
 *
 * This is the single source of truth `ServeRouteGuard` and Serve pages
 * should read from — do not re-fetch or re-derive capabilities elsewhere.
 */
