...
const ServiceRequestPanel: React.FC<Props> = ({ branch }) => {
  ...
  const isOrderPage = pathname.startsWith('/table/') || pathname === '/takeaway';
  ...
  return (
    <div className={cn("fixed right-4 z-40 flex flex-col items-end gap-2", isOrderPage ? "bottom-28" : "bottom-4")}>
      {isOpen && (
        <div className="w-80 max-w-[90vw] rounded-lg border border-border bg-white shadow-lg">
          <div className="flex items-center justify-between border-b border-border p-3">
            <h2 className="text-sm font-semibold text-gray-900">Service Requests</h2>
            <button
              type="button"
          ...
          <div className="max-h-80 overflow-y-auto">
            {count === 0 ? (
              <p className="p-4 text-center text-sm text-gray-500">No open requests</p>
            ) : (
              requests.map((request) => (
                <div
                  key={request.name}
                  className="flex items-center justify-between gap-2 border-b border-gray-100 p-3 last:border-b-0"
                >
      ...
      <button
        ...
        className={cn(
          ...
          count > 0 ? 'bg-amber-500 text-white' : 'bg-white text-gray-600 border border-border'
        )}
      >
