...
def parse_legacy_sites(raw_sites: str) -> List[str]:
	...
	return [site.strip().strip("`").strip('"').strip("'") for site in raw_sites.split(",") if site.strip()]
...
def start_prod(
	...
	if not os.path.exists(env_file_path):
		...
		with open(os.path.join(os.path.expanduser("~"), f"{project}-passwords.txt"), "w") as en:
	...
	with open(compose_file_name, "w") as f:
		try:
			command = [
				...
				"-f",
				("overrides/compose.https.yaml" if is_https else "overrides/compose.noproxy.yaml"),
				"-f",
...
def add_common_parser(parser: argparse.ArgumentParser):
	...
	parser.add_argument("-m", "--http-port", help="Http port in case of no-ssl", default="8080")
...
def add_develop_parser(subparsers: argparse.ArgumentParser):
	...
	parser.add_argument("-n", "--project", default="frappe", help="Compose project name")
...
if __name__ == "__main__":
	...
	if args.subcommand != "exec" and args.force_pull and os.path.exists(get_frappe_docker_path()):
