...
- QZ Printer
        - Add your certificate and private key to `site_config`:
          ```bash
          bench --site your-site set-config qz_cert "$(cat path/to/cert.pem)"
          bench --site your-site set-config qz_private_key "$(cat path/to/private-key.pem)"
          ```
        - The certificate and key are now fetched securely from the authenticated backend API at runtime. They are no longer bundled into client-side JavaScript.
- Network Printer
