...
/**
 * Reports live in this `frontend` SPA itself (basename `/ury`, see
 * frontend/src/main.tsx), under `/reports/:slug` (see App.tsx). This
 * ChatWidget instance is mounted inside the app's `<BrowserRouter>` (see
 * frontend/src/App.tsx), so navigation here is a real in-app react-router
 * `navigate()` call per PLAN.md item 5 — never a hard reload. Callers pass
 * the `navigate` function from `useNavigate()` since this module has no
 * hook access of its own.
 */
export function navigateToReportSlug(navigate: (path: string) => void, slug: string) {
