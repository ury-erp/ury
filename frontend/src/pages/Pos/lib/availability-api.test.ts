...
describe('availability-api (pos)', () => {
  ...
  it('caches per (item_code, branch, company) key and does not leak across keys', async () => {
    ...
    const itemAOtherBranch = buildAvailability({ item_code: 'ITEM-A', branch: 'Kochi', available_qty: 9 });
    const itemB = buildAvailability({ item_code: 'ITEM-B', branch: 'Kozhikode', available_qty: 1 });

    callGetMock
      .mockResolvedValueOnce({ message: itemA })
      .mockResolvedValueOnce({ message: itemAOtherBranch })
      .mockResolvedValueOnce({ message: itemB });

    const a1 = await getItemAvailability({ item_code: 'ITEM-A', branch: 'Kozhikode', company: 'URY' });
    // Second call for the exact same key should be served from cache — no new network call.
    const a2 = await getItemAvailability({ item_code: 'ITEM-A', branch: 'Kozhikode', company: 'URY' });
    // Different branch, same item_code -> distinct cache entry, must hit the network.
    const aOtherBranch = await getItemAvailability({ item_code: 'ITEM-A', branch: 'Kochi', company: 'URY' });
    // Different item_code -> distinct cache entry, must hit the network.
    ...
    expect(aOtherBranch).toEqual(itemAOtherBranch);
    expect(aOtherBranch).not.toEqual(a1);
  ...
  it('skipCache always re-calls the live endpoint, bypassing any cached display value', async () => {
    ...
    const second = buildAvailability({ available_qty: 0, sellable: false, reason_code: 'FG_OUT_OF_STOCK' });
